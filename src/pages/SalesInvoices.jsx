...
function SalesInvoices() {
  ...
  const fetchData = async () => {
    try {
      const invRes = await axiosClient.get("/sales-invoices");
      setInvoices(invRes.data);
    } catch {
      setMessage("Failed to load invoices.");
    }
    try {
      const custRes = await axiosClient.get("/customers/all");
      setCustomers(custRes.data);
    } catch {
      // silently ignore customer fetch errors
    }
  };
  ...
  const calcRawTotal = () => items.reduce((sum, it) => sum + (parseFloat(it.unitPrice) || 0) * (parseInt(it.quantity) || 0), 0);
  const calcDiscount = () => { const raw = calcRawTotal(); return raw > 5000 ? raw * 0.10 : 0; };
  const calcTotal = () => calcRawTotal() - calcDiscount();
        ...
        <div style={styles.card}>
          ...
          {showForm && (
            <form onSubmit={handleSubmit} style={styles.formBox}>
              ...
              {calcDiscount() > 0 && (
                <p style={{ margin: "4px 0 2px", color: "#16a34a", fontWeight: "600" }}>
                  Loyalty Discount (10%): -${calcDiscount().toFixed(2)}
                </p>
              )}
              <p style={{ margin: "4px 0 10px", fontWeight: "bold" }}>
                Total: ${calcTotal().toFixed(2)}
                {calcDiscount() > 0 && <span style={{ fontSize: "12px", color: "#16a34a", marginLeft: "8px" }}>10% loyalty discount applied</span>}
              </p>
              ...
              <tbody>
                {invoices.map(inv => (
                  <React.Fragment key={inv.id}>
                    <tr>
                      ...
                      <td style={styles.td}>
                        {inv.discountAmount > 0
                          ? <span style={{ color: "#16a34a", fontWeight: "600" }}>-${inv.discountAmount.toFixed(2)}</span>
                    ...
                    {selected?.id === inv.id && (
                      <tr>
                        <td colSpan="8" style={{ padding: "12px 16px", background: "#f9fafb" }}>
                          <table style={styles.table}>
                            <thead>
                              <tr>
